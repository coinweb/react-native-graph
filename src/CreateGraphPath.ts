...
export function createGraphPath({
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (i === points.length - 1) {
      path.cubicTo(point.x, point.y, point.x, point.y, 4 * point.x, point.y);
    }
  }
  path.lineTo(width + graphPadding, height + graphPadding);
  path.lineTo(0 + graphPadding, height + graphPadding);
  return path;
}
